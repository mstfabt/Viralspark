...
type PostResult = { text: string; score: number }
...
export default function UrlToPostPage() {
  ...
  const handleGenerate = async () => {
    ...
    setErrorMsg('')
    try {
      const res = await fetch('/api/url-to-post', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ url, platforms: selectedPlatforms, lang: locale }),
      })
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl mx-auto">
      <h1 className="text-2xl md:text-3xl font-bold tracking-tight mb-2">{t('urlToPost.title')}</h1>
      <p className="text-gray-500 dark:text-[#a1a1aa] mb-6 text-sm md:text-base">{t('urlToPost.desc')}</p>
      ...
      <div className="bg-white dark:bg-[#13131a] p-6 md:p-8 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-gray-100 dark:border-[#27272a]">
        <input
          type="url"
          className="w-full p-4 md:p-5 text-base md:text-lg bg-gray-50 dark:bg-[#1a1a22] border border-gray-200 dark:border-[#27272a] rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#d62976] focus:border-transparent transition-all placeholder:text-gray-400 dark:placeholder:text-[#52525b]"
          placeholder={t('urlToPost.placeholder')}
          value={url}
        ...
        <button
          ...
          disabled={isLoading || !url || selectedPlatforms.length === 0}
        ...
        >
          {isLoading ? t('urlToPost.loading') : t('urlToPost.button')}
        </button>
      </div>
                ...
                <div className="p-4 md:p-5">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <button
                      ...
                      className="text-xs bg-gray-100 dark:bg-[#1f1f26] hover:bg-gray-200 dark:hover:bg-[#27272a] px-3 py-1 rounded-full transition-colors ml-auto"
                    >
