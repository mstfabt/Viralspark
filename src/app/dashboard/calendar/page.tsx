...
export default function CalendarPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data.limitReached) {
        ...
        setError(data.error || 'Takvim oluşturulamadı.')
      }
    } catch {
  ...
  const handleExport = () => {
    ...
    a.href = url
    a.download = `viralspark-takvim-${days}gun.csv`
    a.click()
  ...
  if (limits.calendarPerMonth === 0) {
    return (
      <div className="p-8 max-w-3xl mx-auto text-center">
        <h1 className="text-3xl font-bold tracking-tight mb-4">İçerik Takvimi</h1>
        <div className="bg-white p-12 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-gray-100">
          ...
          <p className="text-gray-500 mb-6">Tek konudan 7 veya 30 günlük hazır içerik planı oluşturun.</p>
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <h1 className="text-3xl font-bold tracking-tight mb-2">İçerik Takvimi</h1>
      <p className="text-gray-500 mb-8">Tek konudan günlere yayılmış hazır içerik planı oluşturun.</p>
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-gray-100">
        <div className="space-y-4">
          <textarea
            ...
            rows={2}
            placeholder="Örn: Butik kahveci markası için sosyal medya içerik planı..."
            value={topic}
          ...
          >
            {isLoading ? `${days} Günlük Plan Oluşturuluyor...` : `${days} Günlük Plan Oluştur`}
          </button>
        </div>
      </div>
      ...
      {calendar.length > 0 && (
        <>
          <div className="mt-8 flex items-center justify-between">
            <h2 className="text-xl font-semibold">{calendar.length} Günlük Plan</h2>
            {limits.csvExport ? (
              <button
                onClick={handleExport}
                className="text-sm font-medium bg-white border border-gray-200 px-4 py-2 rounded-full hover:bg-gray-50 transition-colors"
              >
                CSV İndir
              </button>
            ) : (
              <span className="text-xs text-gray-400">CSV export Pro planında</span>
            )}
          </div>

          <div className="mt-4 space-y-4">
            {calendar.map((item, idx) => {
              const platform = PLATFORM_STYLES[item.platform] || PLATFORM_STYLES.twitter
              return (
                <div key={idx} className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
                  <div className="flex items-center gap-4 p-5">
                    {/* Date */}
                    <div className="text-center min-w-[70px]">
                      <p className="text-xs text-gray-400 uppercase">{formatDate(item.date)}</p>
                      <p className="text-2xl font-bold">{new Date(item.date).getDate()}</p>
                    </div>

                    {/* Platform Badge */}
                    <div className={`${platform.color} text-white text-xs font-bold px-3 py-1 rounded-full`}>
                      {platform.label}
                    </div>
                  ...
                  <div className="px-5 pb-5 pt-0">
                    <p className="text-sm text-gray-600 whitespace-pre-wrap leading-relaxed bg-gray-50 p-4 rounded-xl">{item.content}</p>
                  </div>
                </div>
              )
            })}
